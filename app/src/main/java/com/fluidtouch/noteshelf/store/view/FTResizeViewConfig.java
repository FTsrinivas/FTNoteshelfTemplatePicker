...
class FTResizeViewConfig {
    ...
    public static final int DEFAULT_SHAPE = FTResizeImageView.Shape.RECTANGLE;
    ...
    private @FTResizeImageView.Shape
    int shape = DEFAULT_SHAPE;

    public static FTResizeViewConfig from(Context context, AttributeSet attrs) {
        final FTResizeViewConfig cropViewConfig = new FTResizeViewConfig();
        ...
        TypedArray attributes = context.obtainStyledAttributes(
                attrs,
                R.styleable.CropView2);

        cropViewConfig.setViewportRatio(
                attributes.getFloat(R.styleable.CropView2_cropviewViewportRatio,
                        FTResizeViewConfig.DEFAULT_VIEWPORT_RATIO));

        cropViewConfig.setMaxScale(
                attributes.getFloat(R.styleable.CropView2_cropviewMaxScale,
                        FTResizeViewConfig.DEFAULT_MAXIMUM_SCALE));

        cropViewConfig.setMinScale(
                attributes.getFloat(R.styleable.CropView2_cropviewMinScale,
                        FTResizeViewConfig.DEFAULT_MINIMUM_SCALE));

        cropViewConfig.setViewportOverlayColor(
                attributes.getColor(R.styleable.CropView2_cropviewViewportOverlayColor,
                        FTResizeViewConfig.DEFAULT_VIEWPORT_OVERLAY_COLOR));

        cropViewConfig.setViewportOverlayPadding(
                attributes.getDimensionPixelSize(R.styleable.CropView2_cropviewViewportOverlayPadding,
                        FTResizeViewConfig.DEFAULT_VIEWPORT_OVERLAY_PADDING));

        @FTResizeImageView.Shape int shape = attributes.getInt(
                R.styleable.CropView2_cropviewShape, FTResizeViewConfig.DEFAULT_SHAPE);
    ...
    public @FTResizeImageView.Shape
    int shape() {
        return shape;
    }

    public void setShape(@FTResizeImageView.Shape int shape) {
        this.shape = shape;
    }
}
