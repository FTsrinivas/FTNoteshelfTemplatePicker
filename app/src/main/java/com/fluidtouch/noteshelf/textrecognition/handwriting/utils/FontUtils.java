...
public final class FontUtils {
    ...
    public static int calculateFontSizeByBoundingRect(Context context, String text, int textWidth, int textHeight) {
        if (textHeight <= 0 || textWidth <= 0) {
            return 12;
        }
        float VERTICAL_FONT_SCALING_FACTOR = 0.9f;
        Paint mTestPaint;
        mTestPaint = new Paint();
        int margin = context.getResources().getDimensionPixelOffset(R.dimen._10dp);
        DisplayMetrics displayMetrics = context.getResources().getDisplayMetrics();
        ;

        // Find target width
        float targetWidth = textWidth - margin * 2;

        float hi = 800;
        float lo = 10;
        float targetTextSizeHorizontal = hi;
        String[] textPerLines = text.split("\n");
        float targetTextSizeVertical = (textHeight - margin * 2) * VERTICAL_FONT_SCALING_FACTOR;
        targetTextSizeVertical = targetTextSizeVertical / textPerLines.length;
        for (int i = 0; i < textPerLines.length; i++) {
            String subText = textPerLines[i];
            while (hi > 10) {
                float size = hi;
                mTestPaint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, size,
                        displayMetrics));
            ...
            if (targetTextSizeHorizontal > lo)
                targetTextSizeHorizontal = lo;
        }

        // Set the text size
        float targetTextSize = Math.min(targetTextSizeVertical, targetTextSizeHorizontal);
        return  ScreenUtil.convertPxToDp(context, (int) targetTextSize);
    }
}
