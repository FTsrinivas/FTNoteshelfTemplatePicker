...
//        }
    ...
    private COSName createOverlayXObject(PDPage page, LayoutPage layoutPage, COSStream contentStream)
    {
        ...
        PDRectangle base = layoutPage.overlayMediaBox;
        xobjForm.setBBox(base);

        PDRectangle mediaBox = aspectFit(base,page.getMediaBox());
        PDRectangle box = new PDRectangle(base.getWidth(),base.getHeight());
        float x = mediaBox.getWidth() - base.getWidth();
        float y = mediaBox.getHeight() - base.getHeight();

        float scale = mediaBox.getWidth()/base.getWidth();
        AffineTransform transform = new AffineTransform();
        transform.translate(-x/2,-y/2);
        transform.scale(scale,scale);
        xobjForm.setMatrix(transform);
    ...
    private PDRectangle aspectFit(PDRectangle inRect,PDRectangle maxRect)
    {
        float originalAspectRatio = inRect.getWidth() / inRect.getHeight();
        float maxAspectRatio = maxRect.getWidth() / maxRect.getHeight();

        PDRectangle newRect = new PDRectangle(maxRect.getWidth(),maxRect.getHeight());
        if (originalAspectRatio > maxAspectRatio) { // scale by width
            float height = maxRect.getWidth() / originalAspectRatio;
            float y = (maxRect.getHeight() - height)/2.0F;
            newRect.setLowerLeftY(y);
            newRect.setUpperRightY(y+height);
        } else {
            float width = maxRect.getHeight() * originalAspectRatio;
            float x = (maxRect.getWidth() - width)/2.0F;
            newRect.setLowerLeftX(x);
            newRect.setUpperRightX(x+width);
        }
        return newRect;
    }
}
