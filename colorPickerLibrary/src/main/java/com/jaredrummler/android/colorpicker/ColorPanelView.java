 ...
 * See the License for the specific language governing peflrmissions and
...
public class ColorPanelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (shape == ColorShape.SQUARE) {
            ...
            if (borderWidthPx > 0) {
                canvas.drawCircle((float) getMeasuredWidth() / 2, (float) getMeasuredHeight() / 2, outerRadius, borderPaint);
            }
            if (Color.alpha(color) < 255) {
                canvas.drawCircle((float) getMeasuredWidth() / 2, (float) getMeasuredHeight() / 2, outerRadius - borderWidthPx, alphaPaint);
            }
            if (showOldColor) {
                canvas.drawArc(centerRect, 90, 180, true, originalPaint);
                canvas.drawArc(centerRect, 270, 180, true, colorPaint);
            } else {
                canvas.drawCircle((float) getMeasuredWidth() / 2, (float) getMeasuredHeight() / 2, outerRadius - borderWidthPx, colorPaint);
            }
        }
    }
