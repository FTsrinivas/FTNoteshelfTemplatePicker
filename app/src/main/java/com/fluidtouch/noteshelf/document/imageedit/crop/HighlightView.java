...
package com.fluidtouch.noteshelf.document.imageedit.crop;
...
public class HighlightView {
    ...
    private static final float OUTLINE_DP = 4f;
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean maintainAspectRatio, OnCropViewChange listener) {
        mListener = listener;
        matrix = new Matrix(m);
    ...
    void growBy(float dx, float dy) {
        ...
        if (r.width() / scale >= 100 && r.height() / scale >= 100) {
            cropRect.set(r);
            drawRect = computeLayout();
            viewContext.invalidate();
        }
    ...
    public interface OnCropViewChange {
        void onCropViewChange();
    }
